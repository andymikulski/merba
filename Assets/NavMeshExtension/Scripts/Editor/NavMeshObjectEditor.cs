...
namespace NavMeshExtension
{
    ...
    public class NavMeshObjectEditor : Editor
    {
        ...
        private const string editOffText = "Click vertices: Select/Deselect\n" +
        ...
        private void CheckCombine()
        {
            ...
            if (script.subPoints.Count == 0) return;
        ...
        private void DrawPolygonOutline()
        {
            ...
            for (int i = 0; i < allPoints.Length; i++)
            {
				...
				Handles.CircleHandleCap(0, allPoints[i], Quaternion.LookRotation(Vector3.up, Vector3.forward), HandleUtility.GetHandleSize(allPoints[i]) * 0.1f, EventType.Repaint);
                #else
				Handles.CircleCap(0, allPoints[i], Quaternion.LookRotation(Vector3.up, Vector3.forward), HandleUtility.GetHandleSize(allPoints[i]) * 0.1f);
				#endif
            }
        }
